-- 1. Мост из Kafka в сырую таблицу (схлопывание UPDATE и DELETE)
CREATE MATERIALIZED VIEW bio_analytics.mv_kafka_to_sales_raw TO bio_analytics.sales_raw AS
SELECT
    -- Если удаление (d), берем старые данные, иначе новые
    ...
    -- В качестве версии берем таймстемп
...
-- 2. Мост из сырых данных в витрину отчетов (с учетом знака удаления)
...
SELECT
    ...
    sum(amount * sign) AS total_amount -- Умножение на sign автоматически вычитает удаленное
